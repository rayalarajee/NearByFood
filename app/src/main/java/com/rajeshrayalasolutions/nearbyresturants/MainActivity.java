...
import com.rajeshrayalasolutions.nearbyresturants.RetrofitClient.APIInterface;
import com.rajeshrayalasolutions.nearbyresturants.RetrofitClient.ApiClient;
...
public class MainActivity extends AppCompatActivity implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    public ArrayList<PlacesDetails_Modal> searchData;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        changeRadius = (Button) findViewById(R.id.changeRadius);
        currentRadius = (TextView) findViewById(R.id.currentRadius);

        currentRadius.setText("Current Radius :" +radius);
        ...
        search_bar.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if(details_modal.size() > 0){
                    ...
                    for(int i=0 ;i<details_modal.size();i++){
                        if(details_modal.get(i).getName().toLowerCase().contains(s)){
                         searchData.add(details_modal.get(i));
                        }
                    }
                    reloaListData(searchData);
                }
        ...
        changeRadius.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                alert.setMessage("Enter Your Radius");
                ...
                alert.setPositiveButton("Change", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int whichButton) {
                        ...
                        String changedRadius = edittext.getText().toString();
                        radius = Integer.parseInt(changedRadius);
                        currentRadius.setText("Current Radius :" + radius);
                    ...
                    switch (root.status) {
                        case "OK":
                            ...
                            for (int i = 0; i < results.size(); i++) {
                                ...
                                placesDetails_modal.setName(info.name);
                                placesDetails_modal.setLocationA(info.geometry.locationA);
                                placesDetails_modal.setPhone_no("");
                                placesDetails_modal.setPhotourl(photourl);
                                placesDetails_modal.setRating(4);
                                details_modal.add(placesDetails_modal);
    ...
    private void reloaListData(final ArrayList<PlacesDetails_Modal> details_modal) {                            PersistanceForData.getInstance().setPlacesDetails_modals(details_modal);
        PersistanceForData.getInstance().setPlacesDetails_modals(details_modal);

        RestaurantListAdapter adapterStores = new RestaurantListAdapter(getApplicationContext(), details_modal, mAddressOutput);
        recyclerView.setAdapter(adapterStores);
        adapterStores.notifyDataSetChanged();
        ((RestaurantListAdapter)adapterStores).setOnItemClickListener(new RestaurantListAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {
                ...
                DBHelper dbHelper=new DBHelper(MainActivity.this);
                dbHelper.insertData(data.get(position).name,data.get(position).getImageFromBb());
                Toast.makeText(MainActivity.this,"Added to favourites"+position,Toast.LENGTH_LONG).show();
